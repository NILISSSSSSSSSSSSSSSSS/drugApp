<template>
  <div class="SelectCounty">
    <Header title="下属县" :back="Wheel.checkReturns('SelectCounty')" :btnParams="btnParams" @getKeyWords="searchByKeyWords" queryType="county"></Header>
    <div v-for="(item, index) in people" :key="index" class="card" @click="goStation(item.county)">
      <p style="font-size: 16px;margin-bottom: 5px;">{{item.county}}</p>
...
export default {
  ...
  methods: {
    goStation(item) {
      _this.Global['SelectedCounty'] = item;
    ...
    getData() {
      ...
      if(_this.obj['name'] === '救助帮扶') {
        options = {
          ...
          method: 'get',
          params: {
            county: _this.Global['AdminParams']['county'],
