<template>
  <div class="SelectVillages">
    <Header title="下属乡" :back="Wheel.checkReturns('SelectVillages')" :btnParams="btnParams" @getKeyWords="searchByKeyWords"></Header>
    <div v-for="(item, index) in people" :key="index" class="card" @click="goDetail(item.villages)">
      <p style="font-size: 16px;margin-bottom: 5px;">{{item.villages}}</p>
...
export default {
  ...
  methods: {
    goDetail(item) {
      _this.Global['SelectedVillages'] = item;
