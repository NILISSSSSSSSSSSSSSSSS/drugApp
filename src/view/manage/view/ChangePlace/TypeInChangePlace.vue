<template>
  <div class="TypeInChangePlace">
    <Header title="变更执行地录入" back="/Manage/ChangePlace?flag=true" :btnParams="btnParams"></Header>
    <mt-cell title="变更区域">
    ...
    <mt-cell v-if="area === 1" title="执行地" value="请选择地点" @click.native="$refs.region.openPopup('value', 'BIG_REGION_DATA', true)" is-link>
      <span style="color: #888" v-if="region.value">{{region.value | killStation}}</span>
    </mt-cell>
    <mt-cell v-else title="执行地" value="请选择地点" @click.native="$refs.picker.openPopup('green')" is-link>
      <span style="color: #888" v-if="picker.green">{{picker.green}}</span>
    </mt-cell>
    <mt-cell title="变更原因" value="请选择" @click.native="$refs.picker.openPopup('reason')" is-link>
    ...
    <!-- <mt-cell title="委托书照片" value="请选择图片" @click.native="$refs.media.chooseMediaType('pic', 'value')">
    ...
    <ImageBox title="委托书照片" :imageUrls="media" :chooseMediaType="Wheel.chooseMediaType(this, 'pic', 'value')"></ImageBox>
    <CustomButton position="bottom" @click.native="submit()">录入</CustomButton>
    <!-- 时间选择Picker -->
            ...
            submit() {
                ...
                var params_postForm = {
                    ...
                    success: function(res) {
                        ...
                        _this.$router.push({path: '/Manage/ChangePlace'});
                    }
                };
